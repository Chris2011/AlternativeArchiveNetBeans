...
public class AltArchiveFile {
    ...
    public AltArchiveFile(File file) throws ZipException, IOException {
        if (file.getName().endsWith(".7z")) {
            _source = new SevenZipSourceHandler(file, file.getName());
    ...
    private final class SevenZipSourceHandler extends CompressSourceHandler {
        SevenZipSourceHandler(File file, String name)
                throws IOException {
            super(file, new SevenZInputStream(file), name);
        }
        ...
        protected InputStream compressStreamForFile() throws IOException {
            return new SevenZInputStream(_file);
        }
    }

    public class SevenZInputStream extends InputStream {
        private static final String ERROR_MSG_MULTIPLE_7Z_STREAMS = "Multiple 7z streams.";
